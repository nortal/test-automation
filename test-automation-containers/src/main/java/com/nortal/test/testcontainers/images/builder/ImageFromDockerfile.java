...
public class ImageFromDockerfile extends LazyFuture<String>
	...
	private void prePullDependencyImages(Set<String> imagesToPull) {
		...
		imagesToPull.forEach(imageName -> {
			try {
				log.info("Pre-emptively checking local images for '{}', referenced via a Dockerfile. If not available, it will be pulled.",
						imageName);
				DockerClientFactory.instance().checkAndPullImage(dockerClient, imageName);
			} catch (Exception e) {
				log.warn("Unable to pre-fetch an image ({}) depended upon by Dockerfile - image build will continue but may fail. Exception message "
